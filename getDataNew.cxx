...
uint16_t geoadrs[NQTP] = { 5, 6, 7, 8, 9, 10, 11 };
...
uint16_t wgeoadrs[wNQTP] = { 4, 5, 6, 7, 8, 9, 10, 11, 12 };

#define NQDC (NQTP-1)
#define TDC_OFFS (NQDC*34)

#define IOREG_ADDR 0x01000000
#define SCALPED_ADDR 0x11000000
#define SCALSPILL_ADDR 0x22000000
...
int main ( int argc, char** argv )
 {
  ...
  bool end_run(false);
  uint32_t numevts, mask4daq;
  uint16_t datasourcemask, actualmask;
  bool takeboth, valid;
  uint32_t val, nchans, ch;
  uint64_t totTime, uSecPerEv(0);

  numevts = (argc >= 2) ? atoi(argv[1]) : MAXEVENTS;
  mask4daq = (argc >= 3) ? atoi(argv[2]) : 0;
  ...
  const uint32_t chan4fast(0), chan4phys(2), chan4peds(4), chan4eoe(14), chan4we(15);

  FILE* rawfile;

  // instance of V560 scaler for ped gate counting event by event
  myV560 scal4ped ( &mc, SCALPED_ADDR );

  // instance of V560 scaler for spill counting
  myV560 scal4spill ( &mc, SCALSPILL_ADDR );
...
#define NREAD 8
       uint32_t address_list[NREAD] = { SCALSPILL_ADDR+0x10+4*chan4we, SCALSPILL_ADDR+0x10+4*chan4eoe, SCALPED_ADDR+0x10+4*chan4fast, SCALPED_ADDR+0x10+4*chan4phys, SCALPED_ADDR+0x10+4*chan4peds, SCALPED_ADDR+0x50, IOREG_ADDR+0x18, IOREG_ADDR+0x16 };
       uint32_t reading_list[NREAD];
       CVAddressModifier am_list[NREAD] = { cvA32_U_DATA };
       CVDataWidth dw_list[NREAD] = { cvD16 };
       CVErrorCodes ec_list[NREAD];

       scal4spill.readcounter ( chan4we, &nSpills );
       scal4spill.readcounter ( chan4eoe, &nSpills );
